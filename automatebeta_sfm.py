...
imgCropBoundaries = (1, 42, 2559, 1159)
			...
			if (verticalRotations == 0 and xrotation == 0) or verticalRotations == 1:
				...
				if paint:
					...
					if teamColours:
						...
						redVMTContents = open(REDVMTFile, 'rb').read()
						redVMTContents2 = open(r'D:\Steam\steamapps\masterchiefet575\team fortress 2\tf\materials\models\weapons\c_items\c_robo_sandwich_light.vmt', 'rb').read()
						bluVMTContents = open(BLUVMTFile, 'rb').read()
						bluVMTContents2 = open(r'D:\Steam\steamapps\masterchiefet575\team fortress 2\tf\materials\models\weapons\c_items\c_robo_sandwich_light_blue.vmt', 'rb').read()
						f = open(REDVMTFile, 'wb')
						f.write(bluVMTContents)
						f.close()
						f = open(r'D:\Steam\steamapps\masterchiefet575\team fortress 2\tf\materials\models\weapons\c_items\c_robo_sandwich_light.vmt', 'wb')
						f.write(bluVMTContents2)
						f.close()
						SendKeys(r'{F5}')
						mouse.sleep(3)
						# Take whiteBG screenshot and crop
						...
						g.write(redVMTContents)
						g.close()
						g = open(r'D:\Steam\steamapps\masterchiefet575\team fortress 2\tf\materials\models\weapons\c_items\c_robo_sandwich_light.vmt', 'wb')
						g.write(redVMTContents2)
						g.close()
					else:
						# Take whiteBG screenshot and crop
						imgWhiteBG = screenshot()
...
if __name__ == '__main__':
	...
	automateDis(model=model,
				...
				paint = False,
				teamColours = True,
				itemName = 'User RagnarHomsar SFM SDK 3D TEST',
				REDVMTFile = r'D:\Steam\steamapps\common\sourcefilmmaker\usermod\materials\models\weapons\c_items\c_robo_sandwich.vmt',
				BLUVMTFile = r'D:\Steam\steamapps\common\sourcefilmmaker\usermod\materials\models\weapons\c_items\c_robo_sandwich_blue.vmt',
				wikiUsername = 'RagnarHomsar',
				wikiPassword = 'not gonna happen, bub'
				)
